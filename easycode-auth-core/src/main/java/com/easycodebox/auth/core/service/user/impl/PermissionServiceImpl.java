...
import static com.easycodebox.common.lang.Strings.*;
...
public class PermissionServiceImpl extends AbstractServiceImpl<Permission> implements PermissionService {
	...
	public Permission load(Long id, Integer projectId, String url) {
		if (id == null && (projectId == null || isBlank(url)))
	...
	public Permission add(Permission permission) {
		...
		if (isNotBlank(permission.getUrl())) {
	...
	public int update(Permission permission) {
		...
		return super.update(sql()
				...
				.upd(R.Permission.url, trimToNull(permission.getUrl()))
				.upd(R.Permission.sort, permission.getSort())
				.upd(R.Permission.icon, trimToNull(permission.getIcon()))
	...
	public DataPage<Permission> page(String parentName, String projectName,
		...
		parentName = trimToNull(parentName);
		projectName = trimToNull(projectName);
		permissionName = trimToNull(permissionName);
		url = trimToNull(url);
	...
	public synchronized void importFromXml(InputStream streams) throws Exception {
		...
		for (int i = 0; i < menu1s.size(); i++) {
			...
			Permission o1 = new Permission(isNotBlank(menu1Id) ? Long.parseLong(menu1Id) : cal1,
					null, menu1Name, pro.getId(), YesNo.YES, menu1Url, menu1Description, menu1Icon);
			if (isNotBlank(menu1Status)) {
				OpenClose st = Enum.valueOf(OpenClose.class, menu1Status);
				o1.setStatus(st);
			}
			if (isNotBlank(menu1Sort)) {
			...
			for (int j = 0; j < menu2s.size(); j++) {
				...
				Permission o2 = new Permission(isNotBlank(menu2Id) ? Long.parseLong(menu2Id) : cal2,
						o1.getId(), menu2Name,
						pro.getId(), YesNo.YES, menu2Url, menu2Description, menu2Icon);
				if (isNotBlank(menu2Status)) {
					OpenClose st = Enum.valueOf(OpenClose.class, menu2Status);
					o2.setStatus(st);
				}
				if (isNotBlank(menu2Sort)) {
				...
				for (int k = 0; k < menu3s.size(); k++) {
					...
					Permission o3 = new Permission(isNotBlank(menu3Id) ? Long.parseLong(menu3Id) : cal3,
							o2.getId(), menu3Name,
							pro.getId(), YesNo.YES, menu3Url, menu3Description, menu3Icon);
					if (isNotBlank(menu3Status)) {
						OpenClose st = Enum.valueOf(OpenClose.class, menu3Status);
						o3.setStatus(st);
					}
					if (isNotBlank(menu3Sort)) {
	...
	private void analysisPermissionTag(Long menuId, Integer projectId, long begin, int multiple, Element menu) throws Exception {
		...
		for (int x = 0; x < permissions.size(); x++) {
			...
			Permission o = new Permission(isNotBlank(id) ? Long.parseLong(id) : begin + x * multiple,
